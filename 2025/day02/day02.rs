...
fn parse_input() -> Vec<(u64, u64)> {
    INPUT.trim().split(',')
        .map(|s| {
            ...
            (a.parse::<u64>().expect("not a valid int"), b.parse::<u64>().expect("not a valid int"))
        })
        .collect()
}
...
fn part1(input: &[(u64, u64)]) -> u64 {
    ...
    for &(l, r) in input {
        for id_int in l..=r {
            ...
            if is_invalid(id_int, get_first_n_digits(id_int, id_len, candidate_len), id_len, candidate_len) {
                res += id_int;
            }
        }
    }
